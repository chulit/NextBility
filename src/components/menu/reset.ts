...
export default function reset() {
  document
    ?.querySelectorAll('.visua11y-agent-selected')
    ?.forEach((el) => el?.classList?.remove('visua11y-agent-selected'));
  ...
  pluginConfig.offset = Array.isArray(pluginDefaults.offset)
  ...
  const $settingsToggle = document.querySelector<HTMLButtonElement>(
    '.visua11y-agent-settings-toggle'
  );
  const $settingsCard = document.querySelector<HTMLElement>('.visua11y-agent-settings-card');
  const $customPaletteTabs = document.querySelectorAll<HTMLButtonElement>(
    '.visua11y-agent-custom-palette-tab'
  );
  const $customPaletteRange = document.querySelector<HTMLInputElement>(
    '.visua11y-agent-custom-palette-range'
  );
  const $customPaletteBars = document.querySelectorAll<HTMLDivElement>(
  ...
  $customPaletteBars.forEach((barContainer) => {
    ...
    barContainer
      .querySelectorAll('.visua11y-agent-contrast-bar')
      .forEach((bar) => bar.classList.remove('is-active'));
  });

  const $contrastCycleButton = document.querySelector<HTMLButtonElement>(
  ...
  if ($contrastCycleButton) {
    ...
    $contrastCycleButton
      ...
      .forEach((bar) => bar.classList.remove('is-active'));
    $contrastCycleButton
      .querySelector('.visua11y-agent-contrast-bars')
  ...
  const $languageSearch = document.querySelector<HTMLInputElement>(
    '.visua11y-agent-language-search'
  );
  if ($languagePanel) {
